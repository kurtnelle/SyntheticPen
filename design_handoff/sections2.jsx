...
/* ============================================================
   Use Cases — list of capabilities with a featured demo on right
   ============================================================ */
const UseCases = () => {
  const cases = [
    {
      id: 'sig',
      label: 'Signatures',
      desc: 'Replay a stored signature into any form, PDF, or signing surface that accepts pen input.'
    },
    {
      id: 'pres',
      label: 'Presentations',
      desc: 'Annotate slides with pre-authored ink. Pre-scripted handwriting on Whiteboard, OneNote, Concepts.'
    },
    {
      id: 'a11y',
      label: 'Accessibility',
      desc: 'Plot vector glyphs as handwriting for users who cannot hold a stylus. Voice → vector → motion.'
    },
    {
      id: 'svg',
      label: 'SVG Replay',
      desc: 'Drop an .svg onto SyntheticPen and watch the pointer trace it across any canvas surface.'
    },
    {
      id: 'auto',
      label: 'Annotation Automation',
      desc: 'Drive QA tooling and design reviews. Repeatable marks for screen capture, demos, and regression.'
    },
    {
      id: 'cnc',
      label: 'Virtual CNC',
      desc: 'Pipe G-code through the planner. Visualize tool paths against any 2D canvas before machining.'
    }
  ];
  ...
  return (
    <section id="use" className="container" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <SectionHeader
        ...
        title="Anywhere the OS accepts a pointer."
        body="SyntheticPen produces native input. Any Windows application that listens for mouse, pen, or stylus events will receive its motion — no plugins, no shims."
      />
      <div ref={ref} className="reveal" style={{
          ...
          {cases.map((cs, i) => (
            <button
              ...
              style={{
                ...
                textAlign: 'left',
                padding: '20px 22px',
                background: i === active ? 'rgba(77, 163, 255, 0.06)' : 'transparent',
                border: 'none',
                borderBottom: i < cases.length - 1 ? '1px solid var(--border)' : 'none',
                borderLeft: i === active ? '2px solid var(--blue)' : '2px solid transparent',
                color: 'var(--ink)',
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
            ...
            >
              <div>
                <div className="mono" style={{
                  fontSize: 10,
                  letterSpacing: '0.2em',
                  color: i === active ? 'var(--blue)' : 'var(--ink-dim)'
                }}>
                  0{i + 1}
                </div>
                <div style={{
              ...
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                style={{ opacity: i === active ? 1 : 0.3, transition: 'opacity 0.2s' }}>
        ...
        <div className="panel" style={{ padding: 0, overflow: 'hidden', minHeight: 460, display: 'flex', flexDirection: 'column' }}>
          <div style={{
            ...
            <span className="mono" style={{ fontSize: 10, color: 'var(--ink-dim)', letterSpacing: '0.2em' }}>
              PREVIEW · {c.label.toUpperCase()}
            </span>
            <div style={{ display: 'flex', gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'rgba(255,255,255,0.15)' }}></span>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'rgba(255,255,255,0.15)' }}></span>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--blue)', boxShadow: '0 0 6px var(--blue)' }}></span>
            </div>
          </div>
...
/* ============================================================
   UseCaseDemo — animated illustration per case
   ============================================================ */
const UseCaseDemo = ({ id }) => {
  const [t, setT] = useStateT(0);
  useEffectT(() => {
    let raf;
    const start = performance.now();
    const loop = (now) => { setT(((now - start) / 4200) % 1); raf = requestAnimationFrame(loop); };
    raf = requestAnimationFrame(loop);
    return () => cancelAnimationFrame(raf);
  }, [id]);

  // Different geometries per case
  const drawings = {
  ...
  const [pt, setPt] = useStateT({ x: 50, y: 130 });
  ...
  return (
    <svg viewBox="0 0 400 240" width="100%" height="100%" style={{ display: 'block', background: 'rgba(10,10,10,0.6)' }}>
      ...
      <text x="14" y="22" fontFamily="JetBrains Mono, monospace" fontSize="9" fill="rgba(166,166,166,0.4)" letterSpacing="1">
        target_app.exe
      </text>
      <text x="14" y="228" fontFamily="JetBrains Mono, monospace" fontSize="9" fill="rgba(166,166,166,0.4)" letterSpacing="1">
        feed 4800 mm/min · {(t * 100).toFixed(0)}%
      </text>

      {/* Reference path */}
      <path ref={pathRef} d={d} fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="1" strokeDasharray="2 3" />
      {/* Drawn portion (glow) */}
      <path d={d} fill="none" stroke="url(#ucInk)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"
        pathLength="1" strokeDasharray="1" strokeDashoffset={1 - t}
        style={{ filter: 'drop-shadow(0 0 6px rgba(77,163,255,0.7))', opacity: 0.65 }} />
      {/* Drawn portion (sharp) */}
      <path d={d} fill="none" stroke="#E8F4FF" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"
        pathLength="1" strokeDasharray="1" strokeDashoffset={1 - t} />

      {/* Plot head */}
      <circle cx={pt.x} cy={pt.y} r="4" fill="#6BE6FF" style={{ filter: 'drop-shadow(0 0 10px #6BE6FF)' }} />
      <g transform={`translate(${pt.x}, ${pt.y})`}>
        <path d="M 0 0 L 0 14 L 4 11 L 6 16 L 8.5 15 L 6.5 10 L 12 10 Z"
          fill="#F5F5F5" stroke="#0A0A0A" strokeWidth="0.5" strokeLinejoin="round" />
      </g>
    </svg>
  );
};
...
/* ============================================================
   Technology — feature table / spec sheet
   ============================================================ */
const Technology = () => {
  const ref = useReveal();
  const rows = [
  ...
  return (
    <section id="tech" className="container" style={{ paddingTop: 80, paddingBottom: 80 }}>
      <SectionHeader
        ...
        title="Built like motion control hardware."
        body="A look-ahead planner, jerk-limited velocity profiles, and a native Win32 input dispatcher. SyntheticPen treats your cursor with the same discipline a CNC controller treats a tool head."
      />
      <div ref={ref} className="reveal" style={{
        display: 'grid',
        ...
        <div className="panel" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{
            ...
            <span className="mono" style={{ fontSize: 10, color: 'var(--blue)', letterSpacing: '0.2em' }}>
              v0.4.2-beta
            </span>
          </div>
          {rows.map(([k, v], i) => (
...
/* Animated jerk-limited velocity profile chart */
const MotionProfile = () => {
  const [t, setT] = useStateT(0);
  useEffectT(() => {
    let raf;
    const start = performance.now();
    const loop = (now) => { setT(((now - start) / 4000) % 1); raf = requestAnimationFrame(loop); };
    raf = requestAnimationFrame(loop);
    return () => cancelAnimationFrame(raf);
  }, []);

  // S-curve velocity over t=[0,1]
  // accel ramp up to 0.3, cruise 0.3-0.7, decel 0.7-1
  const profile = (x) => {
    if (x < 0.25) {
      const u = x / 0.25;
  ...
  const points = Array.from({ length: N + 1 }, (_, i) => {
    const x = i / N;
    const y = profile(x);
    return [pad.l + x * pw, pad.t + (1 - y) * ph];
  });
  const linePath = 'M ' + points.map(p => p.join(' ')).join(' L ');
  const headIdx = Math.floor(t * N);
    ...
    <div className="panel" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="auto" style={{ display: 'block' }}>
        ...
        <g stroke="rgba(255,255,255,0.05)" strokeWidth="0.5">
          {[0, 1, 2, 3, 4].map(i => (
            <line key={'h' + i} x1={pad.l} y1={pad.t + (i / 4) * ph} x2={W - pad.r} y2={pad.t + (i / 4) * ph} />
          ))}
          {[0, 1, 2, 3, 4, 5, 6, 7, 8].map(i => (
            <line key={'v' + i} x1={pad.l + (i / 8) * pw} y1={pad.t} x2={pad.l + (i / 8) * pw} y2={pad.t + ph} />
          ))}
        </g>
        {/* Axes */}
        ...
        {/* glow under */}
        <path d={linePath} fill="none" stroke="rgba(77,163,255,0.35)" strokeWidth="3" strokeLinecap="round"
          style={{ filter: 'drop-shadow(0 0 8px rgba(77,163,255,0.55))' }} />
        {/* main line */}
        ...
        {head && (
          <>
            ...
            <text x={head[0] + 8} y={head[1] - 8} fontFamily="JetBrains Mono, monospace" fontSize="9" fill="#6BE6FF">
              v={profile(t).toFixed(2)}
            </text>
          </>
        )}
  ...
  return (
    <section id="cta" style={{ paddingTop: 100, paddingBottom: 100, position: 'relative' }}>
      <div className="container">
        <div ref={ref} className="reveal panel" style={{
          padding: '64px 56px',
          textAlign: 'center',
          position: 'relative',
          overflow: 'hidden',
          ...
          <div style={{
            ...
            background: 'radial-gradient(ellipse 60% 80% at 50% 100%, rgba(77,163,255,0.12), transparent 60%)',
            pointerEvents: 'none'
          }} />
          <div style={{ position: 'relative' }}>
            <span className="tag" style={{ marginBottom: 28 }}>
              <span className="dot"></span>BETA · WINDOWS 10/11 (X64)
            </span>
            ...
            <p className="subhead" style={{ maxWidth: 520, margin: '20px auto 0' }}>
              Free during beta. No telemetry. Single signed binary.
            </p>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 36, flexWrap: 'wrap' }}>
              <button className="btn-primary">
                Download SyntheticPen
...
const Footer = () => (
  <footer style={{ borderTop: '1px solid var(--border)', padding: '40px 0 32px', position: 'relative', zIndex: 2 }}>
    <div className="container" style={{
      ...
      <div className="mono" style={{ fontSize: 11, letterSpacing: '0.15em', color: 'var(--ink-dim)' }}>
        © 2026 · BUILT FOR PRECISION
      </div>
      <div style={{ display: 'flex', gap: 24 }}>
        {['Docs', 'Changelog', 'GitHub', 'Contact'].map(l => (
          <a key={l} className="nav-link" style={{ fontSize: 11 }}>{l}</a>
        ))}
      </div>
    </div>
  </footer>
...
Object.assign(window, { UseCases, UseCaseDemo, Technology, MotionProfile, CTA, Footer });
