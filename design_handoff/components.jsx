...
/* ============================================================
   Logo mark — a stylized pen-nib drawing a vector node
   ============================================================ */
const LogoMark = ({ className = '', size = 22 }) => (
...
/* ============================================================
   CursorArrow — Windows-style pointer used to "draw" the path
   ============================================================ */
const CursorArrow = ({ size = 22 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" style={{ filter: 'drop-shadow(0 0 6px rgba(107,230,255,0.7))' }}>
    <path d="M3 2 L3 18 L7.5 14 L10 20 L13 18.7 L10.5 12.8 L17 12.8 Z"
...
/* ============================================================
   SignatureCanvas — the hero centerpiece.
   A multi-stroke cursive path drawn by a glowing cursor.
   Each stroke animates its stroke-dashoffset; cursor follows
   via getPointAtLength on a measurement timer.
   ============================================================ */
const SIGNATURE_STROKES = [
  // First name — flowing cursive "A-l-i-s-t-a-i-r"
...
const SignatureCanvas = () => {
  ...
  const [progress, setProgress] = useState(0);      // 0..1 across all strokes
  ...
  useEffect(() => {
    ...
    const DURATION = 5400; // ms to draw entire signature
    const HOLD = 1600;     // ms hold at end
    ...
    const tick = (t) => {
      ...
      // compute cursor position by mapping p across strokes
      ...
      if (Math.floor(elapsed / cycle) !== iter) setIter(Math.floor(elapsed / cycle));
  ...
  // Render dash offsets per stroke based on cumulative progress
  ...
  // Cursor pixel position relative to SVG viewBox -> we render cursor inside SVG to keep alignment
  return (
      ...
      >
        <defs>
          <linearGradient id="inkGrad" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#4DA3FF" className="ink-stop-a" />
            <stop offset="100%" stopColor="#6BE6FF" className="ink-stop-b" />
          </linearGradient>
        ...
        <g fontFamily="JetBrains Mono, monospace" fontSize="9" fill="rgba(166,166,166,0.55)">
          ...
          {[0,1,2,3,4,5,6,7].map(i => (
            <g key={i}>
              <line x1={40 + i*100} y1="240" x2={40 + i*100} y2="244" stroke="rgba(166,166,166,0.25)" strokeWidth="0.6" />
            </g>
          ))}
        </g>

        {/* Signature strokes — drawn twice: glow underlay + crisp top */}
        <g filter="url(#inkGlow)" opacity="0.9">
          {SIGNATURE_STROKES.map((d, i) => (
            <path
              ...
              strokeDashoffset={dashOffsets[i] || lengths[i] || 0}
            />
          ))}
        </g>
        <g>
        ...
        {/* Trail dot */}
        {progress < 1 && (
          <circle cx={coords.x} cy={coords.y} r="5" fill="#6BE6FF" opacity="0.9"
        ...
        {/* Cursor arrow rendered as SVG (positioned via transform) */}
        <g transform={`translate(${coords.x - 1}, ${coords.y - 1})`}>
          <path d="M 0 0 L 0 16 L 4.5 12 L 7 18 L 10 16.7 L 7.5 10.8 L 14 10.8 Z"
            fill="#F5F5F5" stroke="#0A0A0A" strokeWidth="0.5" strokeLinejoin="round"
            style={{ filter: 'drop-shadow(0 0 6px rgba(107,230,255,0.6))' }} />
        </g>
      </svg>

      {/* Bottom strip — progress bar / interpolation status */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
        gap: 16,
        ...
        <div style={{ flex: 1, height: 2, background: 'rgba(255,255,255,0.06)', position: 'relative' }}>
          <div style={{
            ...
            height: '100%',
            width: `${progress * 100}%`,
            background: 'linear-gradient(90deg, #4DA3FF, #6BE6FF)',
            boxShadow: '0 0 8px rgba(107,230,255,0.6)'
          }} />
        </div>
...
/* ============================================================
   Background spline curves — drifting decorative curves
   ============================================================ */
const BackgroundSplines = () => (
  ...
  >
    ...
    <path d="M -100 200 C 200 100, 400 350, 700 300 S 1200 450, 1600 250"
      fill="none" stroke="url(#splineGrad)" strokeWidth="1" />
    <path d="M -100 600 C 300 700, 600 500, 900 650 S 1300 800, 1600 600"
      fill="none" stroke="url(#splineGrad)" strokeWidth="1" />
    <path d="M -100 400 C 250 350, 500 600, 850 500 S 1300 350, 1600 450"
      fill="none" stroke="rgba(107,230,255,0.12)" strokeWidth="0.8" />
    {/* Control points */}
    {[[200, 100], [400, 350], [700, 300], [1200, 450]].map(([x, y], i) => (
      <g key={i}>
    ...
    <section style={{ position: 'relative', paddingTop: 56, paddingBottom: 80, overflow: 'hidden' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 28 }}>
          <span className="tag"><span className="dot"></span>v0.4 BETA · WINDOWS 10/11</span>
        </div>
        ...
        <p className="subhead" style={{ textAlign: 'center', maxWidth: 640, margin: '24px auto 0' }}>
          Synthetic cursor &amp; pen motion for Windows. SyntheticPen replays SVG paths
          as native input — like a CNC plotter for your handwriting.
        </p>
...
Object.assign(window, { LogoMark, CursorArrow, Header, SignatureCanvas, Hero, BackgroundSplines });
