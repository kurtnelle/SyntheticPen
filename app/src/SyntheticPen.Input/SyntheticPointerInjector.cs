...
#if DEBUG
            // Rich one-shot JSON dump for field diagnosis. Debug builds only —
            // Release (Store) ships none of this; the concise throw below is
            // the only failure surface there.
...
#if DEBUG
    /// <summary>
    /// Debug-only diagnostic ring buffer of recent injected pointer events
    /// with monotonic timestamps. On the first injection failure it writes a
    /// one-shot JSON dump (failure context + recent timeline) next to the
    /// executable so inter-stroke timing can be inspected. Entirely compiled
    /// out of Release builds — the Store binary has no diagnostics.
    /// </summary>
